...
npatch <- 100
z <- sapply(1:npatch, function(i) ar2.sim.tv(n=50, a0=a0.t, a1=a1.t, a2=a2.t, sd=0.2)$ts.x)
...
ar2.plot()
for(i in 1:npatch) ar2.arrows(1 + vc.fit[[i]][[1]][,"fit"], vc.fit[[i]][[2]][,"fit"], col=i, length=0)
...
for (i in 1:npatch) {
	time <- start(dat[[i]])[1]:end(dat[[i]])[1]
	b.gam.fit <- vc.fit[[i]]
	points(time, 1 + b.gam.fit[[1]][,"fit"],col=i,pch=1,type="l")
}
...
#for(i in 1:npatch) {
#	ar2.vc.gam.plot(x=vc.fit[[i]],fit="R")
#	title(names(gr.fox)[[i]])
#	}
