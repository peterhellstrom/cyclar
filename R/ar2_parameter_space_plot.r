...
#' @export
ar2.dsotm <- function(
    a1 = NULL, a2 = NULL, n = 100,
    add.series = TRUE, col.series = "darkgreen", main = NULL,
  ...
  if (is.null(main)) {
    title(main = list("Why Pink Floyd?", col = "white"))
  } else {
    title(main = list(main, col = "white"))
  }

  # Add triangle
  # points(c(-1, 0, 1, -1), c(-1, 1, -1, -1), type = "l")
  curve(-x^2, from = -1, to = 1, add = TRUE, n = 1001, lty = 2, col = "darkgrey")
  ...
  # Add triangle
  if (!scatter.smooth) {
    points(c(-1, 0, 1, -1), c(-1, 1, -1, -1), type = "l", col = "white")
  }
  ...
  if (add.series) {
    ...
    if (is.null(a1) | is.null(a2)) {
      series <- ts(log(lynx), start = 1)
    }
    if (!is.null(a1) && !is.null(a2)) {
      series <- ar2.sim2(n = n, mu = 0, a1 = a1, a2 = a2)
    }
